...
# Minimum tested Fusion 360 version (NR-9). This is informational — we log a
# warning if the running version appears older, but don't block startup.
TESTED_FUSION_VERSION = "2.0.20440"


def _log_fusion_version():
    """Log the Fusion 360 version and warn if it appears older than the tested baseline (NR-9)."""
    try:
        version = app.version
...
def _cleanup_stale_files():
    """Remove stale command/response files from previous sessions (NR-2).

    Called on startup to prevent processing leftover files from crashed sessions.
    Files are safe to remove because the MCP server generates fresh command IDs.
    """
    stale_count = 0
    for pattern in ("command_*.json", "response_*.json", "command_*.tmp", "response_*.tmp"):
        for f in COMM_DIR.glob(pattern):
            try:
                f.unlink()
                stale_count += 1
            except Exception:
                pass
    if stale_count > 0:
        logger.info("Cleaned up %d stale IPC files from previous session", stale_count)
...
def run(context):
    ...
    try:
        ...
        # Log Fusion 360 version for diagnostics (NR-9)
        _log_fusion_version()
        ...
        try:
            ...
            for cmd_file in cmd_files:
                try:
                    ...
                    # Protocol version validation (NR-1) — reject mismatched versions
                    cmd_version = command.get("protocol_version")
                    if cmd_version != PROTOCOL_VERSION:
                        logger.warning(
                            "Protocol version mismatch for command %s: expected %d, got %s",
                            command_id, PROTOCOL_VERSION, cmd_version,
                        )
                        _handle_cmd_error(
                            cmd_file,
                            f"Protocol version mismatch: add-in expects {PROTOCOL_VERSION}, "
                            f"command has {cmd_version}. Update both MCP server and add-in.",
                        )
                        continue
